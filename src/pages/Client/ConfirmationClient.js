...
export default class ConfirmationClient extends React.Component {
	constructor(props) {
		super(props)
		console.log(Date.parse(this.props.navigation.state.params.responseJson.start_date))
	...
	async endTrip() {
		try {
			...
			Alert.alert(
				...
				[
					{text: "OK", onPress: ()=> this.props.navigation.navigate('SignatureClient', {responseJson})}
				],
			)
		} catch (error) {
			console.error(error)
