...
export default class DriverTrip2 extends React.Component {
	constructor(props) {
		super(props)
	...
	async endTrip() {
			try {
			let url = API_URL+'/driver-trip/'+this.state.trip_id
			console.log(url)
			let response = await fetch(
				url,
				{
			...
			this.props.navigation.navigate("ConfirmationDriver", {responseJson})
		} catch (error) {
	...
	async cancelTrip() {
		try {
			let response = await fetch(
				API_URL+'/driver-trip/'+this.state.trip_id, {
					method: "DELETE",
					headers: {
						'Content-Type': 'application/json'
					}
